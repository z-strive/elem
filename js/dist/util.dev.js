...
function url(key) {
  ...
  var obj = {}; // 数组.forEach(function(item,index){}) 数组的循环
...
function formatTime(time) {
  var t = new Date(time); //获取年

  var year = t.getFullYear(); //获取月份

  var month = t.getMonth() + 1; //获取日

  var day = t.getDate(); //获取时

  var hour = t.getHours(); //获取分

  var min = t.getMinutes(); //获取秒

  var sec = t.getSeconds();
  return "".concat(year, "-").concat(addZero(month), "-").concat(addZero(day), " ").concat(addZero(hour), ":").concat(addZero(min), ":").concat(addZero(sec));
} //封装函数实现数据之前补零操作
